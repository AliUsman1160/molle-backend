  ...
  if (dt_user_table.length) {
    ...
    , {
        ...
        render: function render(data, type, full, meta) {
          ...
          var $row_output = '<span class="name">' + $name + '</span>';
          return $row_output;
        }
      }, {
        ...
        targets: 3,
        render: function (data, type, full, meta) {
            var formattedDate = moment(full['created_at']).format('YYYY-MM-DD');
            return '<span class="created_at">' + formattedDate + '</span>';
        }
      ...
      }, 
    {
        ...
        orderable: false,
        render: function render(data, type, full, meta) {
          return '<div class="d-inline-block text-nowrap">' + "<button class=\"btn btn-sm btn-icon edit-record\" data-id=\"".concat(full['id'], "\" data-bs-toggle=\"offcanvas\" data-bs-target=\"#offcanvasAddUser\"><i class=\"mdi mdi-pencil-outline mdi-20px\"></i></button>") + "<button class=\"btn btn-sm btn-icon delete-record\" data-id=\"".concat(full['id'], "\"><i class=\"mdi mdi-delete-outline mdi-20px\"></i></button>")  +  '</div>';
        }
      }],
      order: [[2, 'desc']],
      ...
      buttons: [{
        ...
        text: '<i class="mdi mdi-plus me-sm-1"></i><span class="d-none d-sm-inline-block">Add New Event Type</span>',
...
// Form submission
var addNewEventtypeForm = document.getElementById('addNewEventtypeForm');

// user form validation
var fv = FormValidation.formValidation(addNewEventtypeForm, {
  fields: {
    name: {
      validators: {
        notEmpty: {
  ...
  $.ajax({
    data: $('#addNewEventtypeForm').serialize(),
    url: url,
    type: method,
